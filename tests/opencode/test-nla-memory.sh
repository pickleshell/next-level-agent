#!/usr/bin/env bash
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
node "$SCRIPT_DIR/test-nla-memory.mjs"
node "$SCRIPT_DIR/test-nla-mem0.mjs"
node "$SCRIPT_DIR/test-nla-mem0-skill.mjs"
node "$SCRIPT_DIR/test-nla-compaction.mjs"
node "$SCRIPT_DIR/test-nla-compaction-boundary.mjs"
node "$SCRIPT_DIR/test-nla-request-boundary.mjs"
node "$SCRIPT_DIR/test-nla-child-recovery.mjs"
node "$SCRIPT_DIR/test-nla-utility-runtime.mjs"
node "$SCRIPT_DIR/test-nla-prompt-optimizer.mjs"
node "$SCRIPT_DIR/test-nla-capability-cache.mjs"
