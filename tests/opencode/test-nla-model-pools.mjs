...
assert.equal(modelPoolSummary({ roles: { explorer: { enabled: true, models: ['fixture/model'] } } })[0].status, 'on');
assert.equal(modelPoolSummary({ roles: { explorer: { enabled: false, models: ['fixture/model'] } } })[0].status, 'off');
...
try {
  ...
  const changedPolicy = await plugin.tool.nla_model_policy.execute({ role: 'architect', policy: 'balanced' }, context);
  assert.equal(changedPolicy.metadata.policy, 'balanced');
  assert.equal(changedPolicy.metadata.cost_weight, undefined);
  ...
  assert.deepEqual(JSON.parse(savedPolicy.output).value, { selection_policy: 'balanced', minimum_score: 7.5 });
