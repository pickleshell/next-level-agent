...
export function validateModelPools(parsed, source = 'model pool file') {
  ...
  for (const [role, pool] of Object.entries(parsed.roles)) {
    ...
    if (pool.selection_policy !== undefined && !['quality', 'balanced', 'cost', 'local', 'free'].includes(pool.selection_policy)) throw new ModelPoolValidationError(`Role ${role}.selection_policy must be quality, balanced, cost, local, or free in ${source}`, source);
    if (['local', 'free'].includes(pool.selection_policy) && !['select', 'auto'].includes(pool.selection_mode)) throw new ModelPoolValidationError(`Role ${role}.selection_policy ${pool.selection_policy} requires select or auto in ${source}`, source);
